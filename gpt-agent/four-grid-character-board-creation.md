...
3. 正面服装细节图优先由用户从母图裁切；用户要求时可单独生成。
4. 输出独立单图，由用户自行拼接。
...
4. 图像生成工具明确返回不可用、失败、报错或限流时，使用更短的当前步骤提示词重试一次。
5. 第二次明确失败后，输出“当前步骤恢复提示词”，供用户在任意可生成图片的客户端重新发送。
6. 回复中的图片、路径和生成状态必须对应真实结果。

当前步骤恢复提示词格式：

```text
图像生成工具连续失败，下面是当前步骤的恢复提示词，可在任意可生成图片的客户端重新发送：
...
本节只在用户明确指定某种风格时启用。默认保持真实真人电影质感。
...
- 用户明确指定风格时，将该风格作为视觉约束。
- 用户只上传风格参考图但未说明是否套用时，先简短确认。
- 角色板任务始终保持母图工作流、单图输出和无文字水印。
- 用户明确风格优先；未明确时保持真实真人电影质感。
...
5. `image_04_front_outfit_detail.png`，仅在用户需要时生成；默认建议用户从母图裁切。
...
- 图像生成失败、报错或被限流时，使用更短的当前步骤提示词重试一次。
- 第二次明确失败后停在当前步骤，输出当前步骤恢复提示词。
- 用户事后报告某个客户端卡住时，下一次默认用更短的当前步骤提示词直接生成一次；明确失败后再输出恢复提示词。
